...
- Functional conversion path: in-repo **GTM** + **Termly** embed and **Consent Mode** defaults are wired; **full** analytics and attribution validation (tags firing after consent, **Understory** **`dataLayer`**) completes in **Phase 8** (**`docs/phase-8-todo.md`** **P8-07**, **P8-22**).

## Phase 8, Deployment and production cutover

- **Pre-cutover:** Dev and staging (**`https://andetag-web.guff.workers.dev`**) pass full functional QA (routing, scripts, cookies, consent, embeds, forms). **GTM** + **Termly** container migration (**`docs/gtm-termly-migration-runbook.md`**, **P8-07**) is complete and Preview-validated on staging before **`www`** cutover. Must-keep URLs from the legacy live site behave per **`docs/url-matrix.csv`** and **`docs/url-migration-policy.md`**; exceptions are approved and logged.
