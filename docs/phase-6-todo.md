...
| **P6-01** (English Stockholm + English global) | **Code complete:** **`PAGE_CUSTOM_BODY_PATHS`** lists **46** routes with custom bodies (**23** English + **23** Swedish). **`/en/`** is a **header-selector shell** (city choice only); it is **not** in that set. **Blocking P6-01 closure:** Gustaf **package** sign-off (chrome + hub + bodies) and English copy pass. |
| **P6-02** onward | **Not started** until **P6-01** is signed off in the verification record. **P6-02** is **`en` + Berlin** chrome and **`/en/berlin/...`** bodies. Then **P6-03** (German), **P6-04**–**P6-06** (metadata QA, exceptions log, Phase 6 doc closure). |

**Your next actions (Gustaf):** Review Wave 1 English in the browser; record **P6-01** package approval in **`docs/phase-6-verification-record.md`** when satisfied. **Agent next actions** after that: mark **P6-01** checklist item complete here, then start **P6-02** per the wave rules below (one route at a time, **`npm test`** / **`npm run build`** green between pages).
...
- **Content build order (strict):**
  1. **English Stockholm** (`**/en/stockholm/...**`) **together with** **English global** pages (`**/en/<global-slug>/...**` per matrix, not under `/stockholm/` or `/berlin/`).
  2. **English Berlin** (`**/en/berlin/...**`), including **new** content where there is no Berlin counterpart yet.
  3. **German Berlin** (`**/de/berlin/...**`) **together with** **German global** pages (`**/de/...**`), by **translating** the **new** Berlin-facing (and global) English copy (and using Swedish or sources where that is the authority).
...
- **Swedish chrome:** Update **`/sv/...`** header/footer/nav to the **same** destination + language model where applicable so the shell is coherent (body copy remains Phase 5 approved unless a small exception is logged).
...
**Rollout order:** **`sv` / Stockholm** **first** (legacy rename, new chrome, existing **`/sv/...`** bodies wired through the new shell), **package-approved** by Gustaf; then **Wave 1** full build (**`en` / Stockholm** chrome + **`/en/`** hub + **`/en/stockholm/...`** + English global), **package-approved**; **Wave 2** full build (**`en` / Berlin** chrome + **`/en/berlin/...`**), **package-approved**; **Wave 3** full build (**`de` / Berlin** chrome + **`/de/berlin/...`** + German global), **package-approved** (German copy still gets external review pre-launch per **Language review** above).
...
- [ ] **P6-01** **Wave 1 — English Stockholm + English global:** **Implementation ready for Gustaf inspection (2026-03-24).** In-scope English Stockholm + English global bodies (matrix shells except **`/en/berlin/`**, **P6-02**) use **`site/src/lib/page-registry/page-body-registry.ts`** (**`PAGE_CUSTOM_BODY_PATHS`**, **46** paths) and **`site/src/pages/[...slug].astro`**. **`/en/`** hub uses **`chrome-hdr-en-header-selector`** (no entry in **`PAGE_CUSTOM_BODY_PATHS`**). **`/en/stockholm/`** **`StockholmHomeEn.astro`** and shell metadata close **EX-0007**. **`npm test`** and **`npm run build`** in **`site/`** green. **Check this item when:** verification record §P6-01 shows **package sign-off: Yes**. **Until then:** your review. See **`docs/phase-6-verification-record.md`**.
- [ ] **P6-02** **Wave 2 — English Berlin:** Build **`en` + Berlin** hero, small header, footer **and** **`/en/berlin/...`** bodies (including **new** pages where Berlin parity requires it). **Bodies:** **one page at a time** with tests, build, and fixups before the next. **Then** **Gustaf design-approves the full Wave 2 package** (chrome + bodies). **English copy review:** Gustaf.
- [ ] **P6-03** **Wave 3 — German Berlin + German global:** Build **`de` + Berlin** hero, small header, footer **and** **`/de/berlin/...`** plus **German global** **`/de/...`** bodies; translate from Wave 2 English (and matrix). **Bodies:** **one page at a time** with tests, build, and fixups before the next. **Then** **Gustaf design-approves the full Wave 3 package** (chrome + bodies). **German:** external language reviewer **once**, late **pre-launch**; track internal QA until then.
