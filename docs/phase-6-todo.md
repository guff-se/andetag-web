...
| **P6-01** (English Stockholm + location-scoped English story URLs) | **Code complete:** **`PAGE_CUSTOM_BODY_PATHS`** lists **54** routes (**29** English + **24** Swedish + **1** German privacy). Story topics (about, artists, music, optical fibre) live under **`/en/stockholm/{slug}/`** and **`/sv/stockholm/{slug}/`** (not legacy global **`/en/music/`**-style URLs; those **`301`** to Stockholm). **`/en/`** is a **header-selector shell** (city choice only); it is **not** in that set. **Routing slice (2026-03-28):** **`docs/routing-location-scoped-global-pages-plan.md`** (implemented): matrix (**61** shells), **`_redirects`**, registry **`hreflang`**, Berlin English story **`seoCanonicalPath`**, four privacy shells, **`en-brand`** removed. **Blocking P6-01 closure:** Gustaf **package** sign-off (chrome + hub + bodies) and English copy pass. |
| **P6-02** onward | **Not started** until **P6-01** is signed off in the verification record. **P6-02** is **`en` + Berlin** chrome polish and **remaining** **`/en/berlin/...`** bodies (hub and story or privacy shells already route; some bodies reuse Stockholm English components). Then **P6-03** (German), **P6-04**–**P6-06** (metadata QA, exceptions log, Phase 6 doc closure). |
...
- [ ] **P6-01** **Wave 1 — English Stockholm + location-scoped story URLs:** **Implementation ready for Gustaf inspection** (last wiring pass **2026-03-28**, routing slice). In-scope English Stockholm bodies and **four** story topics under **`/en/stockholm/{slug}/`** use **`site/src/lib/page-registry/page-body-registry.ts`** (**`PAGE_CUSTOM_BODY_PATHS`**, **54** paths) and **`site/src/pages/[...slug].astro`**. **`/en/`** hub uses **`chrome-hdr-en-header-selector`** (no entry in **`PAGE_CUSTOM_BODY_PATHS`**). Swedish counterparts are under **`/sv/stockholm/...`**. **`/en/stockholm/`** **`StockholmHomeEn.astro`** and shell metadata close **EX-0007**. **`npm test`** and **`npm run build`** in **`site/`** green. **Check this item when:** verification record §P6-01 shows **package sign-off: Yes**. **Until then:** your review. See **`docs/phase-6-verification-record.md`**.
- [ ] **P6-02** **Wave 2 — English Berlin:** Build **`en` + Berlin** hero, small header, footer **and** **remaining** **`/en/berlin/...`** bodies (story and privacy shells may already render reused Stockholm English bodies from the **2026-03-28** routing delivery; finish any gaps and **package** chrome + content). **Bodies:** **one page at a time** with tests, build, and fixups before the next. **Then** **Gustaf design-approves the full Wave 2 package** (chrome + bodies). **English copy review:** Gustaf.
