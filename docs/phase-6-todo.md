...
- [ ] **P6-01** **Wave 1 — English Stockholm + English global:** **Implementation ready for Gustaf inspection (2026-03-24).** All in-scope English Stockholm and English global bodies (matrix shells except **`/en/berlin/`**, **P6-02**) are wired through **`PAGE_CUSTOM_BODY_PATHS`** and **`[...slug].astro`** (**47** migrated paths including **`/sv/...`**). **`/en/stockholm/`** body **`StockholmHomeEn.astro`** and shell metadata close **EX-0007**. **`npm test`** and **`npm run build`** in **`site/`** green. **Outstanding:** your **package** design sign-off (chrome + hub + bodies) and English copy review. Sources: **`page-shell-meta.json`**, **`docs/url-matrix.csv`**, **`docs/phase-4-route-coverage.md`**, **`site-html/`** English. Prior work: **`/en/`** hub (**`EnglishHubEn.astro`**); English Stockholm chrome (**`hero-en-stockholm.ts`**, **`footer-en-stockholm.ts`**); **`navigation.ts`** **`en-main`**. See **`docs/phase-6-verification-record.md`**.
