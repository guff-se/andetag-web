...
- [ ] **P6-00** **Chrome architecture, legacy rename, and `sv` / Stockholm package:** Define the **8 headers + 4 footers** model (**Header and footer variant matrix** above). Replace legacy **header** and **footer** WordPress ids (see **Legacy naming** above) with **stable** names and a documented **old → new** map. Implement **destination + language** controls (**all** languages and **both** cities visible), **single** URL-resolution path (coupling rules in **Decided**; normative copy **`docs/phase-4-routing-reopen.md`**). Update **`docs/content-model.md`**, shell registry, **`layoutVariantsForPath`**, **`SiteHeader`**, **`SiteLayout`**, **`page-shell-meta`**, LCP preloads, and tests. Complete **`sv` + Stockholm** hero, small header, and footer **and** integration with existing **`/sv/...`** bodies under the new shell; **then Gustaf design-approves the full Swedish package** (chrome + content). **After** that approval, start Wave 1 build.
- [ ] **P6-01** **Wave 1 — English Stockholm + English global:** Build **`en` + Stockholm** hero, small header, and footer; **`/en/`** **minimal** hub; **`/en/stockholm/...`** bodies (including **`/en/stockholm/`** metadata **EX-0007**); **English global** routes; nav/footer labels. **Bodies:** **one page at a time** with tests, build, and fixups before the next (**Decided**). **Then** **Gustaf design-approves the full Wave 1 package** (chrome + hub + bodies). **English copy review:** Gustaf. Sources: **`page-shell-meta.json`**, **`docs/url-matrix.csv`**, **`docs/phase-4-route-coverage.md`**, `site-html/` English.
