...
| Consent | **`necessary`** only: must not be gated behind optional CMP consent categories; document in the CMP (CookieConsent) as essential / functional (aligned with `docs/tracking-and-consent-requirements.md`). |
...
4. **Entry URLs:** include **`/sv/stockholm/`**, hubs, and inner pages per matrix **`keep`** rules; **`/`** and **`/en/`** are **routing** URLs ( **`302`** / hub **`200`** per policy). The sitemap should reflect **policy** and **`docs/Andetag SEO Manual.md`** (for example whether the hub is indexable); align with Gustaf before **`www`** if ambiguous. The static Astro sitemap excludes the root URL **`https://www.andetag.museum/`** because **`site/src/pages/index.astro`** is **only** a **`301`** to **`/sv/stockholm/`** (not a separate indexable document).
