...
| Artwork medium line (label + construction, sv/en) | `site/src/lib/content/artwork-medium.ts` (consumed by the artwork modal and both per-artwork page bodies; the short schema.org `artMedium` form stays in `schema-org.ts`) |
