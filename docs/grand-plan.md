...
**Status (2026-03-28):** **P6-00** **closed** (Gustaf sign-off): Swedish **`sv` / Stockholm** chrome package (stable ids, **`resolveChromeNavigationHref`**, selectors, hero destination row) approved. Evidence: **`docs/phase-6-verification-record.md`**. **P6-01** (English Stockholm + location-scoped story URLs and privacy): **implementation complete** (**54** custom body paths in **`PAGE_CUSTOM_BODY_PATHS`**; **`/en/`** hub is a header-selector shell and is not in that set). **Routing alignment:** **`docs/routing-location-scoped-global-pages-plan.md`** **implemented** same date (matrix **61** shells, single-hop legacy **`301`**, Berlin English story **`seoCanonicalPath`**, **`en-brand`** removed). **Ready for Gustaf package inspection**; stakeholder sign-off still open. **Next:** **`docs/phase-6-todo.md`** (**Current position and what is next**), then **P6-02** (Berlin English **package** chrome and any remaining bodies).
