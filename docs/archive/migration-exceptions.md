...
| EX-0010 | 2026-03-23 | 5 | content | Legacy WP **`mailto:info@tadaa.se`** in scraped **`site-html/`** (for example **`stockholm-dejt.html`**) | Public contact is **`mailto:info@andetag.museum`**: **`BookingEmbed`** **`.booking-embed-contact`** (**`booking-embed-contact.ts`**, sv/en/de), Stockholm home and SEO landings, FAQ accordions, group and corporate CTAs, NPF and accessibility copy, English FAQ, JSON-LD **`email`** (**`schema-org.ts`**), press footer line. Visible addresses use **`mailto:`** links. | Align public inbox with the ANDETAG domain; **2026-04-16** extended beyond booking-adjacent copy | none | All listed contact paths open **`info@andetag.museum`** | approved (Gustaf) | none |
...
| EX-0017 | 2026-04-06 | 7 | SEO | Legacy Yoast JSON-LD vs rebuilt JSON-LD in **`site/src/lib/chrome/schema-org.ts`** | Entity-first graph: Stockholm shells emit **`@type`: [`Museum`, `LocalBusiness`]**: **`Museum`** for semantics; **`LocalBusiness`** (paired only here, **2026-04-14**) so nested **`aggregateRating`** / **`Review`** satisfy Google's review-snippet list of valid parent types (**`Museum`** alone is not listed). Still no **`TouristAttraction`**. Museum **`url`** follows shell language (**`/sv/stockholm/`** vs **`/en/stockholm/`**). **`aggregateRating`** / featured **`Review`** (TripAdvisor figures: **`stockholm-reviews.ts`** only), **6 `Offer`** nodes (**`stockholm-offers.ts`**), **four dated `Event` nodes** for Art Yoga (**`computeArtYogaOccurrenceSeriesIso`**, build-time). **`FAQPage`** on **`/en/stockholm/faq/`** and **`/sv/stockholm/fragor-svar/`** only (**`stockholm-faq.ts`**, **2026-04-23**). Berlin pre-opening: **`Place`** only. Legacy **`ArtGallery`** dropped. | Align with **`docs/Andetag SEO Manual.md`** §6 and §11; `aggregateRating` per §6; `offers` from centralized price data (stable for 1+ year); `Event` uses `eventSchedule` for recurring Art Yoga | low | Star ratings, price range, and event rich results eligible in SERP for Stockholm pages; legacy standalone **`ArtGallery`** no longer emitted; FAQ rich-eligible on FAQ URLs | approved (Gustaf, 2026-04-12) | **Post-cutover (P8-26):** monitor Rich Results in GSC for 2-4 weeks. **Phase 9 (P9-14):** skill for maintaining review data in **`stockholm-reviews.ts`** and prices in **`stockholm-offers.ts`**. |
