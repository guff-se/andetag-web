...
| EX-0006 | 2026-03-22 | 3 | component | `/component-showcase/` | Accept Lighthouse Performance below 90 on mobile for the internal showcase while Accessibility meets or exceeds 95 | Showcase intentionally stacks all embeds, scripts, and media on one `noindex` route for single-pass sign-off, which dominates performance metrics versus production pages | none | none for end users, internal QA route only | approved (Gustaf) | Revisit after consent-gated lazy embed patterns in Phase 7; migrated pages use Phase 5 perf targets in `docs/definition-of-done.md` |
| EX-0007 | 2026-03-23 | 4 | content | `site-html/en-stockholm.html` canonical points to `/` | Phase 4 shell for `/en/stockholm/` uses `<title>` and `og:description` from `site-html/en.html` via `page-shell-meta.json` | No dedicated snapshot exposes correct head metadata for `/en/stockholm/` without fabricating copy | low | none until Phase 5 replaces shell with migrated body and source-backed metadata | pending | Replace with extraction from a corrected crawl or parser output when available |
