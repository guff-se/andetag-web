...
| EX-0007 | 2026-03-23 | 4 | content | `site-html/en-stockholm.html` and live `https://www.andetag.museum/en/stockholm/` (verified 2026-03-23: Yoast still emits Swedish home title, description, and canonical pointing at Swedish home) | Phase 4 shell for `/en/stockholm/` uses `<title>` and `og:description` from `site-html/en.html` via `page-shell-meta.json` | Live and scraped HTML both mis-target SEO for this path; no faithful English hub metadata available without editorial or parser fix | low | none until Phase 6 replaces shell with migrated body and source-backed metadata | approved (Gustaf, Phase 4 closure 2026-03-23) | Resolve in **Phase 6** with English localization (stakeholder deferral 2026-03-24); not Phase 5 |
