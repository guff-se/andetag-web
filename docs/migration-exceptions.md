...
| EX-0017 | 2026-04-06 | 7 | SEO | Legacy Yoast JSON-LD in **`site-html/en-stockholm-tickets.html`**, **`site-html/en.html`**, **`site-html/de-berlin.html`**, **`site-html/de-ueber-andetag.html`** vs rebuilt JSON-LD in **`site/src/lib/chrome/schema-org.ts`** and built **`site/dist/.../index.html`** | Keep the rebuilt entity-first graph contract and do not mirror Yoast schema parity: Stockholm shells expose **`WebSite`**, **`WebPage`**, **`Organization`**, **`ImageObject`**, **`Museum`**, **`TouristAttraction`**; Berlin pre-opening shells expose **`WebSite`**, **`WebPage`**, **`Organization`**, **`ImageObject`**, **`Place`** only. Legacy **`ArtGallery`**, **`LocalBusiness`**, and Stockholm **`Event`** types are dropped. Legacy rating and commerce properties (**`aggregateRating`**, **`review`**, **`offers`**) are not emitted in rebuilt Stockholm venue nodes. | Align with **`docs/Andetag SEO Manual.md`** §6 and §11 (Berlin pre-opening protocol), reduce noisy or stale plugin-driven schema, and keep deterministic schema generation in app code instead of Yoast side effects | medium | Search appearance can shift because eligible rich result classes and optional stars or offer snippets may differ from legacy pages | pending | Re-check representative **`https://www.andetag.museum/...`** URLs in Google Rich Results after Phase 8 cutover, confirm no required-property errors, and either approve this entry or revise **`schema-org.ts`** |
| EX-0018 | 2026-04-06 | 7 | tracking | Shared **`GTM-KXJGBL5W`**: legacy **`www`** WordPress + Complianz vs static staging + **Termly** | Finish **GTM** trigger migration off **`cmplz_*`** mainly at **`www`** cutover (**Phase 8**); **brief analytics or marketing gaps** on migration day are acceptable if GTM publish timing straddles CMP or host switch | Same container on both stacks until cutover; dual triggers optional for continuity (**`docs/kpi-measurement-map.md`** § Staged rollout, § GTM migration checklist) | low | Possible short discontinuity in measured traffic or conversions around cutover | approved (Gustaf) | **`docs/phase-8-todo.md`** **P8-13**, **P8-22**; verify **Understory `dataLayer`** after static **`www`** |
