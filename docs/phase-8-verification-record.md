...
- **P8-04 Exception sign-off:** **Approved 2026-04-12.** All active exceptions (**EX-0002** through **EX-0019**) are now **approved** or **resolved**. **EX-0017** revised: Stockholm venue schema includes `aggregateRating`, featured `Review` nodes, **`Offer`** nodes, and Art Yoga **`Event`** with **`Schedule`**, per **`stockholm-reviews.ts`** / **`stockholm-offers.ts`** (TripAdvisor headline figures in code only, not duplicated here). Legacy **`ArtGallery`** and **`LocalBusiness`** types dropped per SEO manual §6 and §11.
