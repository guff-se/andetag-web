...
- **P8-22 Live feature pass:** **Partial 2026-04-14.** **CMP + consent + tags** already **P8-12**/**P8-13**. **Lighthouse** mobile performance-only on **four** template URLs (**`BASE_URL=https://www.andetag.museum`**, **`LIGHTHOUSE_PATHS`** comma list); see table below (local repro: **`site/reports/lighthouse-performance.json`**; **`site/reports/*.json`** is **`.gitignore`**d). **Booking / conversion on `www`:** operator confirms execution **2026-04-14** (§Manual post-cutover checkpoints); maintainer logs outcomes here, not in chat-only form. **Header locale switch** on **`www`**: step **5** in §Manual post-cutover checkpoints. **Still open:** other **P8-01** items (embeds/forms spot checks not yet listed).
