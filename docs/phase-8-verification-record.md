...
- **P8-04 Exception sign-off:** **Approved 2026-04-12.** All active exceptions (**EX-0002** through **EX-0019**) are now **approved** or **resolved**. **EX-0017** revised: Stockholm venue schema includes `aggregateRating` and featured `Review` nodes sourced from **`stockholm-reviews.ts`** (TripAdvisor figures maintained in code only, not duplicated here); `ArtGallery`, `LocalBusiness`, `Event`, `offers` remain intentionally dropped per SEO manual §6 and §11.
