...
|----|-------|--------|-------|-------|
| M-0001 | Centralise Stockholm opening hours | open | skills/operational-facts | Added 2026-04-24. Hours are duplicated in `site/src/components/page-bodies/OppettiderSv.astro`, `OppettiderEn.astro`, and `STOCKHOLM_OPENING_HOURS` in `site/src/lib/chrome/schema-org.ts`. Consider a single `STOCKHOLM_HOURS` export in `stockholm-offers.ts` (or a new `stockholm-hours.ts`) with both schema-ready `OpeningHoursSpecification[]` and localised prose strings; consumed by both page bodies and `schema-org.ts`. Track drift between prose and schema as the driver. |
...
| Id | Title | Status | Owner | Notes |
|----|-------|--------|-------|-------|
| M-0003 | Collapse SEO-landing `testimonialItems` onto `STOCKHOLM_FEATURED_REVIEWS` | done | skills/testimonials | Added 2026-04-24. Done 2026-05-08. Replaced literal inline arrays in `StockholmSeoLandingSv.astro`, `StockholmSeoLandingEn.astro`, and `StockholmHomeSharedBody.astro` with `.map(r => ({ quote: r.quote, author: r.author }))` from the catalogue. Also updated `STOCKHOLM_FEATURED_REVIEWS` with 5 real TripAdvisor reviews (taur:…) selected for conversion and SEO; updated aggregate metrics (reviewCount=197, fiveStarReviewCount=183). `skills/testimonials/SKILL.md` updated to reflect auto-propagating consumers and the CSV selection workflow. |
