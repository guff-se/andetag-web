...
- **JSON-LD Rich Results (Stockholm):** **`Museum`** node uses a single **`@type`** (removed **`TouristAttraction`** array entry) so **Google** accepts **`aggregateRating`** and **`Review`**. Art Yoga emits **four** dated **`Event`** nodes (next four Tuesdays) with computed **`startDate`** / **`endDate`**, **`image`**, distinct **`@id`**, and offer **`validFrom`** (**`computeArtYogaOccurrenceSeriesIso`** in **`art-yoga-next-occurrence.ts`**). **`schema-org.ts`**, **`schema-org.test.ts`**, **`art-yoga-next-occurrence.test.ts`**, **`docs/migration-exceptions.md`** **EX-0017**.
