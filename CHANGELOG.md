...
- **Site structure side phase (pre-execution lock):** **`docs/decisions/0003-site-src-structure.md`** (**Accepted**) records final rename targets (`components/chrome`, `lib/chrome`, `components/page-bodies`, `lib/page-registry`, `lib/ui-logic`, `site/src/client-scripts`). **`docs/phase-structure-todo.md`** is the S0–S8 checklist (baseline hash filled by the executing agent). **`docs/site-structure-refactor-plan.md`** status **ready for execution**, **agent-only** execution (no stakeholder input), fixed **CHANGELOG** policy and **S7** skipped by default, §6.1 stale-path **`rg`**, §8 risks updated. **`docs/decisions/README.md`** examples list **`0003`**. **`AGENTS.md`** documentation table + code layout pointer. **Why:** one maintainer agent can run the full refactor without blocking on optional human choices.
