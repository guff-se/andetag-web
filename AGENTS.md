...
- **English is British English.** All English copy uses UK spelling and vocabulary: `-ise`/`-isation` (never `-ize`, and not Oxford `-ize` either), `-yse`, `-our`, `-re`, `-ogue`, doubled `-ll-`, `programme` for events, `towards`, plus lift / toilet / metro / pushchair over the American equivalents. Full tables and the exclusion list are in `docs/Tone of Voice.md` §Spelling. This governs prose only: code identifiers, CSS properties, schema.org and Consent Mode keys, URLs, vendored files under `skills/design/`, and verbatim review quotes keep their existing spelling. Before committing: `cd site && npm run lint:copy`.
