...
Cleanly roll back a recent content change using Git — creating a new commit that inverts the regression, opening a pull request, verifying the revert on a Cloudflare preview, and merging to promote the fix to `www`. The skill enforces the maintenance-era release discipline: no direct pushes to `main`, every change lands via PR + preview + merge.
