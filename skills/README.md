...
| [`operational-facts`](operational-facts/SKILL.md) | P9-18 | active | Update Stockholm operational facts (hours, prices, daytime window, contact email, address, geo) with the correct propagation set. Flags which facts are centralized (prices, season pass, daytime window, Art Yoga metadata, address, geo) vs not (opening hours, contact email) and lists every file that must change together. |
...
| [`design`](design/SKILL.md) | — | active | **Mandatory for all UI/visual design work.** Vendored [Trystan-SA/claude-design-system-prompt](https://github.com/Trystan-SA/claude-design-system-prompt) (MIT): `system-prompt.md` + 14 procedures in `procedures/`. ANDETAG overrides in `andetag-adaptations.md`; brand from `docs/Visual Identity.md`. Production, extract, and review procedures (discovery, wireframe, token extract, accessibility, ai-slop-check, polish-pass). |

Rows reflect the active maintenance skill set. The numeric ids carry from the original Phase 9 work items.
