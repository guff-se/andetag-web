...
6. **Source artifacts.** `site-html/` is the **legacy** WP scrape — reference only. `SEO-0019` (utställning spelling) and the migration-only `EX-0007` (en-stockholm Yoast drift, in `docs/migration-exceptions.md` until archive) document where the scrape cannot be trusted. Always prefer `page-shell-meta.json` + `stockholm-*.ts` + `schema-org.ts` over `site-html/`.
