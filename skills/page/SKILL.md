...
The six existing landings (per `docs/Andetag SEO Manual.md` §17.1):
...
| Meditation | `/sv/stockholm/meditation-stockholm/` | `/en/stockholm/meditation-stockholm/` |
...
6. **Footer surface (not main nav).** Add an entry to the `seoLinks` array in `site/src/lib/chrome/footer-sv.ts` and `site/src/lib/chrome/footer-en-stockholm.ts`. Use a one- or two-word label aligned with the H1 keyword (existing labels: `Aktivitet` / `Activity`, `Museum`, `Att göra` / `Things to do`, `Event`, `Utställning` / `Exhibition`, `Meditation` / `Meditation`). Do **not** edit `NAVIGATION_VARIANTS` in `site/src/lib/chrome/navigation.ts` — landings are deliberately absent from the main menu.
