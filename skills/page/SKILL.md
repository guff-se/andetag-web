...
| Area | File | Notes |
|------|------|-------|
| Source HTML (meta extraction) | `archive/legacy-wordpress-site/site-html/*.html` | Frozen WP mirror (read-only). Used only to seed `<title>` and `<meta name="description">` when you run `npm run page-shell:meta`. For a brand-new page, add minimal `<head>`-only stubs here (or edit `page-shell-meta.json` + `docs/meta-texts-catalog.md` directly per `skills/seo`). |
...
1. **Source HTML** — create `archive/legacy-wordpress-site/site-html/stockholm-new-page.html` and `archive/legacy-wordpress-site/site-html/en-stockholm-new-page.html`. Each needs only a minimal `<head>` with `<title>` and `<meta name="description">` matching the copy you want in the shell. Alternatively, add the two paths to `page-shell-meta.json` and `docs/meta-texts-catalog.md` and skip the extractor. **Do not fabricate a title or description that does not reflect the page's real content.**
...
1. Create `archive/legacy-wordpress-site/site-html/stockholm-skolgrupper.html` and `archive/legacy-wordpress-site/site-html/en-stockholm-school-groups.html` with a minimal `<head>` (or add shell rows to the meta JSON + catalog).
