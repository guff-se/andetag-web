...
| Chrome navigation (`write`) | `site/src/lib/chrome/navigation.ts` **and** `site/src/lib/chrome/hero-{sv,en-stockholm,en-berlin,de-berlin}.ts` | Two sources of truth. `NAVIGATION_VARIANTS` in `navigation.ts` covers ids `sv-main`, `en-main`, `en-main-berlin`, `de-main`. The rendered desktop + mobile header reads `MENU_DEFINITION` (or the inline `menuItems` array on the Berlin heros) in the four `hero-*.ts` files — `SiteHeader.astro` uses `sharedHero.menuItems`, NOT `model.navigation`. **Edit both.** `nav-parity.test.ts` enforces agreement on the descendant link set. **SEO landing pages do not go in either** — they belong in the footer `seoLinks` strip below. |
...
- **Wordmark scan** — For page-body copy edits that touch brand mentions, verify new/edited occurrences of `ANDETAG` are rendered with `<span class="brand-wordmark">ANDETAG</span>` where markup supports it (headings, paragraphs, links), not just capitalised plain text.
